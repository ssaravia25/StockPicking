...
@st.cache_data(ttl=3600, show_spinner="Running backtest... Calculating 2-year performance.")
...
# Initialize Engines
engine = get_engine()
metrics_engine = get_metrics_engine_v3()
...
<div style="text-align: center; color: #888; font-size: 0.8em; padding: 20px;">
    ...
    <span style="font-size: 0.7em; opacity: 0.6;">Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Data coverage: 2 years</span>
</div>
""", unsafe_allow_html=True)
